...
async def list_drivers(active: Optional[bool] = None, current_user=Depends(get_current_user)):
    db = get_supabase()
...
async def create_driver(driver: DriverCreate, current_user=Depends(get_current_user)):
    db = get_supabase()
...
async def update_driver(driver_id: UUID, driver: DriverUpdate, current_user=Depends(get_current_user)):
    db = get_supabase()
...
async def deactivate_driver(driver_id: UUID, current_user=Depends(get_current_user)):
    db = get_supabase()
