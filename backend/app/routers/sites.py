...
async def list_sites(active: Optional[bool] = None):
    db = get_supabase()
...
async def get_site(site_id: UUID):
    db = get_supabase()
...
async def update_site(site_id: UUID, site: SiteUpdate):
    db = get_supabase()
...
async def deactivate_site(site_id: UUID):
