...
async def list_contractors(active: Optional[bool] = None, current_user=Depends(get_current_user)):
    db = get_supabase()
...
async def create_contractor(contractor: ContractorCreate, current_user=Depends(get_current_user)):
    db = get_supabase()
...
async def update_contractor(contractor_id: UUID, contractor: ContractorUpdate, current_user=Depends(get_current_user)):
    db = get_supabase()
