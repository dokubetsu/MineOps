...
async def site_dashboard(site_id: UUID, for_date: Optional[date] = None):
    ...
    # Contractor breakdown for trips
    by_contractor = {}
    for t in trips:
...
@router.get("/owner")
async def owner_dashboard(from_date: Optional[date] = None, to_date: Optional[date] = None):
    ...
    # Latest closing balance per site
    latest_cash = {}
    for cb in cash_data:
        sid = cb["site_id"]
    ...
    sorted_trend = sorted(trip_trend.items())

    return {
        "period": {"from": str(from_date), "to": str(to_date)},
        "total_trips": len(trips),
        "sites": site_summaries,
        "trip_trend": [{"date": d, "count": c} for d, c in sorted_trend],
    }


@router.get("/stakeholder/{user_id}")
async def stakeholder_dashboard(user_id: UUID):
    """Stakeholder read-only dashboard - their sites and revenue share"""
    db = get_supabase()
    # Get their site access
    ...
    for access in accesses:
        ...
        # Aggregated cash (stakeholder view)
